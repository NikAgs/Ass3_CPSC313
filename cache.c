...
/*
 * Initialize a new cache line with a given block size. This function
 * is used as a helper by the cache_set_init function.
 */
 ...
 * size. This function is used as a helper by the cache_new function.
 */
...
/*
 * Helper function that is used to compute the shift and mask given
 * the number of bytes/sets.  You do not need to worry about this
 * function.
 */
 ...
 * operations. Possible values of polices are listed in file cache.h.
 */
...
/*
 * Determine whether or not a cache line is valid and its tag maches
 * the given tag. Returns 1 if both conditions are satisfied, 0
 * otherwise.
 */
...
/*
 * Return the integer that is at offset "offset" inside a cache
 * line. This function assume that the cache line is valid, and that
 * tags match (it does not need to check this again).
 */
 ...
 * given index is tagged as the most recently used one. When we use a
 * LRU replacement policy (the only one where this function should be
 * called), the most recently used cache line will be the 0'th one in
 * the set, the second most recently used cache line will be next,
 * etc. Cache lines whose valid bit is 0 will occur after all cache
 * lines whose valid bit is 1.
 */
...
/*
 * Retrieve a matching cache line from a set, if one exists. This
 * function is responsible for verifying that the cache line it
 * returns is valid and that the tags match. This function returns
...
/*
 * Read a single integer from the cache. The parameter "address"
 * contains the memory address of the int64_t to be read.
 */
